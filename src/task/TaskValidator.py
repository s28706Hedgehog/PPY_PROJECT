...
class TaskValidator:
    ...
    def validate_terminate_task(cls, task: 'Task'):
        if task.state is TaskState.FINISHED:
            raise InvalidStateChangeException("Can't terminate task that's already finished.")
        if task.state is TaskState.TO_DO:
            raise InvalidStateChangeException("Can't terminate task that is not even running. Start it first")
